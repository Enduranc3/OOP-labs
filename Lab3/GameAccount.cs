...
public abstract class GameAccount(string userName, int currentRating)
{
    ...
    protected Dictionary<Guid, int> RatingHistory(IGameRepository gameRepository) => gameRepository.ReadAll()
        .Where(game => game.Player1Id == UserId || game.Player2Id == UserId)
        .ToDictionary(game => game.GameId, game => game.GetSignedRatingChange(this));
    ...
    protected static string GetSafePlayerName(PlayerService playerService, IGameAccountRepository repository, Guid playerId)
    {
        return playerService.ReadAccountById(playerId)?.UserName ?? "Unknown Player";
    }

    public virtual void PrintStats(PlayerService playerService, IGameAccountRepository repository, IGameRepository gameRepository)
    {
        Console.WriteLine(
    ...
    : GameAccount(userName, currentRating)
{
    ...
    public override void PrintStats(PlayerService playerService, IGameAccountRepository accountRepository, IGameRepository gameRepository)
    {
        Console.WriteLine(
